...
class NPIRequestsRepository extends BaseRepository
{
  ...
  SELECT
    ...
    npi_request_temp.check_employee_credential_id,
...
INNER JOIN `check_employee_credentials` ON `check_employee_credentials`.`id` = `$this->table`.`check_employee_credential_id`
INNER JOIN `checks_credential_matches` ON `checks_credential_matches`.`check_id` = `check_employee_credentials`.`check_id`
INNER JOIN `credential_matches` ON `credential_matches`.`id` = `checks_credential_matches`.`credential_match_id`
WHERE 
`$this->table`.check_employee_credential_id = '" . $item['check_employee_credential_id'] . "' AND
